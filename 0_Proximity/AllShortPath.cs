...
{
    public class AllShortPath : GH_Component
    {

        public AllShortPath()
          : base(
                "All Short Path",//Full Name
                "All Path",//Nick Name
                "Find all shortest paths from start point to all nodes in the entire network using Dijkstra algorithm.\n\n© Zach X.G. Zheng | Zachitect.com",//Description
                "Zachitect.com",//Tab
                "Proximity"//Category
                )
        {
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddPointParameter("Destination", "Destination(s)", "Optional destination point", GH_ParamAccess.list);
        ...
        {
            pManager.AddCurveParameter("Shortest Paths", "Al Paths", "All shortest paths from departure to all node in the entire network", GH_ParamAccess.list);
            pManager.AddNumberParameter("Shortest Distances", "Distances", "Distances of all shortest paths", GH_ParamAccess.list);
            pManager.AddPointParameter("Unique Nodes", "Unique Nodes", "Unique nodes of the entire network", GH_ParamAccess.list);
            pManager.AddIntegerParameter("Departure Index", "Start Index", "Index of departure point in unique nodes", GH_ParamAccess.item);
            pManager.AddIntegerParameter("Destination Index", "End Index", "Index of destination point(s) in unique nodes", GH_ParamAccess.list);
            pManager.AddIntegerParameter("Index of Curves", "Network Index", "Index of network curves in each shortest path", GH_ParamAccess.tree);
            pManager.AddIntegerParameter("Index of Nodes", "Node Index", "Index of nodes of each shortest path", GH_ParamAccess.tree);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            List<Point3d> EP = new List<Point3d>();
            DA.GetDataList(2, EP);
            ...
            /*if (EP.Count > 0)
            {
                ...
                {
                    if RoundPoint(SP).DistanceTo(RoundPoint(EP[i])
                }
            ...
            DA.SetDataList(0, OuputShortest);
            DA.SetDataList(1, Distances);
            DA.SetDataList(2, UniqueNodes);
            DA.SetData(3, SPIndex);

            if (EP.Count > 0)
            {
                int[] EPIndexList = new int[EP.Count];
                for (int i = 0; i < EP.Count; i++)
                {
                    int EPIndex = GetSource(UniqueNodes, EP[i]);
                    EPIndexList[i] = EPIndex;
                }
                DA.SetDataList(4, EPIndexList);
            }

            Grasshopper.Kernel.Data.GH_Structure<GH_Integer> outTree = new Grasshopper.Kernel.Data.GH_Structure<GH_Integer>();
            for (int i = 0; i < EdgePathRecords.Length; i++)
            {
                Grasshopper.Kernel.Data.GH_Path path = new Grasshopper.Kernel.Data.GH_Path(i);
                foreach(int index in EdgePathRecords[i])
                {
                    GH_Integer GI = new GH_Integer(index);
                    outTree.Append(GI, path);
                }
            }

            DA.SetDataTree(5, outTree);

            Grasshopper.Kernel.Data.GH_Structure<GH_Integer> outTreeNode = new Grasshopper.Kernel.Data.GH_Structure<GH_Integer>();
            for (int i = 0; i < NodePathRecords.Length; i++)
            {
                Grasshopper.Kernel.Data.GH_Path path = new Grasshopper.Kernel.Data.GH_Path(i);
                foreach (int index in NodePathRecords[i])
                {
                    GH_Integer GI = new GH_Integer(index);
                    outTreeNode.Append(GI, path);
                }
            }
            DA.SetDataTree(6, outTreeNode);
...
protected override System.Drawing.Bitmap Icon
        {
            get
            {
                ...
                return Properties.Resources.AllShortPath;
            }
        }
        public override Guid ComponentGuid
        {
            get { return new Guid("676c1538-467f-4af5-a14e-72408877f67e"); }
        }
    }
}
